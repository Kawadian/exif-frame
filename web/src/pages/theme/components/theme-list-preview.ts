...
type PreviewTask = {
  ...
  generation: number;
  photo: Photo;
  onResult: (themeName: string, dataUrl: string | null) => void;
};
...
const buildScaledInput = (options: ThemeOption[] | undefined, themeName: string, scale: number): ThemeOptionInput => {
  const input: ThemeOptionInput = new Map();
  options?.forEach((option) => {
    const saved = Customize.get(themeName, option.id, getConverter(option.type));
    let value: string | number | boolean = saved !== null ? saved : option.default;
...
const photoKeyOf = (photo: Photo): string => `${photo.file.name}:${photo.file.size}:${photo.file.lastModified}`;

class ThemeListPreviewQueue {
  ...
  enqueue(themeName: string, photo: Photo, onResult: PreviewTask['onResult'], prioritize = false) {
    ...
    this.queue = this.queue.filter((task) => !(task.themeName === themeName && task.photoKey === photoKey));
...
const themeListPreviewQueue = new ThemeListPreviewQueue();

export { themeListPreviewQueue, photoKeyOf };
